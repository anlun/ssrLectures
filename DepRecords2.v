...
Definition prodPre {A B : poset} (x y : A * B): bool :=
  let: (xa, xb) := x in
  let: (ya, yb) := y in
  pre xa ya && pre xb yb.
...
case y => ya yb.
rewrite /prodPre.
move /andb_prop; case => H1 H2.
move /andb_prop; case => H3 H4.
...
Definition funcPre {A : Type} {B : poset} (f1 f2 : A -> B): bool :=
